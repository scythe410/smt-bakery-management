...
// Reports error boundary (DESIGN.md §6): state what happened and the fix, with a
  ...
  return (
    <Card className="flex flex-col items-start gap-3">
      <div className="flex flex-col gap-1">
        <p className="text-h2 text-ink">{t("reports.error.title")}</p>
        <p className="text-body text-muted">{t("reports.error.body")}</p>
      </div>
      <button
        type="button"
        onClick={reset}
