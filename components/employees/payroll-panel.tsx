...
function PayrollRow({ row, payDate }: { row: PayrollDayEmployee; payDate: string }) {
  ...
  return (
    <div className="border-border flex flex-col gap-2 border-t py-3 first:border-t-0 first:pt-0">
      <div className="flex items-start justify-between gap-2">
        <div className="flex flex-col gap-0.5">
          <span className="text-label text-ink font-medium">{row.name}</span>
        ...
        {paid && row.payment ? (
          ...
          <StatusPill tone="warning" label={t("employees.payroll.status.pending")} />
        ) : null}
      </div>
        ...
        <div className="flex items-end gap-2">
          ...
          {row.payment ? (
            <button
              ...
              disabled={isPending}
              aria-label={t("employees.payroll.delete")}
              className="text-danger h-9 shrink-0 transition-colors disabled:opacity-40"
            >
              <Trash2 className="size-4" aria-hidden />
            </button>
          ) : null}
